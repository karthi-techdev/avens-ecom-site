...
//     // --- Combined Robust Filtering & Sorting Logic ---
...
import { useState, useEffect, useMemo } from 'react';
// Import useSearchParams to read the URL
...
export default function ProductShowPage() {
    ...
    const searchTerm = searchParams.get('search') || ""; // Get the word from the URL bar
    ...
    const filteredAndSortedProducts = useMemo(() => {
        ...
        // 1. FILTER BY SEARCH TERM (This is what was missing)
        if (searchTerm) {
            items = items.filter(product => 
        ...
        // Added searchTerm to the dependency array below
    }, [products, sortOption, priceRange, selectedCategoryId, searchTerm]); 
                    ...
                    <div className={`col-span-12 ${isSidebarOpen ? "lg:col-span-9" : "lg:col-span-12"} transition-all duration-300`}>
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center py-20 bg-gray-50 rounded-2xl">
                                ...
                                <button 
                                    onClick={() => {
                                        window.history.pushState({}, '', '/product-list'); // Clear search from URL
                                        setSelectedCategoryId(null);
                                        setPriceRange([0, 2000]);
                                    }} 
                                    className="text-[#3BB77E] underline font-bold mt-2"
