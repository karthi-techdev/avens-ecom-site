        ...
        <section className={`!px-[1rem] relative !py-[0.8rem] sm:!px-[2rem] sm:!py-[1rem] md:!px-[4rem] md:!py-[1rem] lg:!px-[6rem] lg:!py-[2rem]`}>
            <div className='flex justify-between items-center'>
                <div className='grid grid-cols-3 gap-[0.6rem]'>
                    <button type='button' className={`${activeTab === 'feature' ? 'bg-[var(--orange-light)] ml-1' : 'bg-[var(--border-color)] ml-1'} rounded-md !py-[0.4rem] !px-[1.2rem] text-[1.1rem] font-semibold capitalize ${activeTab === 'feature' ? 'text-[var(--primary)]' : ''} hover:-translate-y-2 transition-all duration-500 hover:text-[var(--primary)] hover:bg-[var(--orange-light)] cursor-pointer`} onClick={() => setActiveTab('feature')}>Featured</button>
                    ...
                    <button type='button' className={`${activeTab === 'newone' ? 'bg-[var(--orange-light)] ml-1' : 'bg-[var(--border-color)] ml-1'} rounded-md !py-[0.4rem] !px-[1.2rem] text-[1.1rem] font-semibold capitalize ${activeTab === 'newone' ? 'text-[var(--primary)]' : ''} hover:-translate-y-2 transition-all duration-500 hover:text-[var(--primary)]  hover:bg-[var(--orange-light)] cursor-pointer`} onClick={() => setActiveTab('newone')} >New added</button>
                </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 !mt-[2rem] gap-[1.5rem]'>
                {products.filter((item) => item.type === activeTab).map((prod, index) => (
                    <ProductCard key={index} product={prod} onQuickView={onQuickView} />
                ))}
            </div>
        </section>
    );
