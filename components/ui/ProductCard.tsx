...
const ProductCard = ({ product, onQuickView, view = 'grid' }: ProductCardProps) => {
    ...
    const detailUrl = `/product-view/${product?.slug || product?._id}`;

    // --- Image Logic ---
    const baseUrl = (URLs.FILEURL || "http://localhost:5000").replace(/\/$/, "");
    const rawPath = product?.thumbnail || product?.images?.[0] || product?.image || "";
    ...
    const addToCart = async (e: React.MouseEvent) => {
        ...
        try {
            await addCart({
                ...
                price: finalPrice, // Keeping API consistent
                productId: product._id,
                userId: token._id,
                product: product // Keeping the full product object for store logic
            });
            await getAllCart(token._id);
            toast.success('Added to cart!', { transition: Bounce });
        } catch (error: any) {
            toast.warn(error.message || "Failed to add to cart");
        }
    };
    ...
    const IconButton = ({ icon: Icon, label, color, onClick, onDoubleClick }: any) => (
            ...
            <div className="group relative w-full bg-white rounded-2xl overflow-hidden transition-all duration-300 flex flex-col md:flex-row p-4 sm:p-6 border border-[var(--border-color)] hover:shadow-lg">
                ...
                <div className="relative w-full md:w-64 aspect-square overflow-hidden rounded-xl bg-white cursor-pointer p-3" onClick={handleProductClick}>
                    <div className="w-full h-full transition-transform duration-700 ease-in-out group-hover:scale-110">
                        <img src={finalImageSrc} alt={product?.name} className="w-full h-full object-contain" />
                        {finalHoverImageSrc && (
                            <img src={finalHoverImageSrc} alt={product?.name} className="absolute inset-0 w-full h-full object-contain opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                        )}
                    </div>
                ...
                <div className="flex-1 mt-4 md:mt-0 md:ml-8 flex flex-col justify-center">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        {hasDiscount && (
                            <>
                                ...
                                <span className="text-red-500 font-bold text-sm">{discountPercent}% OFF</span>
                            </>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-1 mb-4">
                        {[...Array(5)].map((_, i) => (
                            <MdStarPurple500 key={i} className={i < Math.round(rating) ? "text-yellow-400" : "text-gray-300"} />
                        ))}
                    ...
                    <button className="w-fit px-6 py-2 bg-[var(--primary)] text-white rounded-lg flex items-center gap-2 hover:bg-[#29A56C] transition-colors" onClick={addToCart}>
        ...
        <div className="group/card relative border border-gray-200 rounded-[20px] p-4 bg-white hover:shadow-lg transition-all duration-300 h-full flex flex-col">
            {mounted && showRemoveAlert && createPortal(
                <div className="fixed top-5 right-5 z-[9999] bg-white border-l-8 border-red-500 shadow-xl p-4 rounded flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <h4 className="text-gray-900 font-bold text-sm">Removed!</h4>
                        <p className="text-gray-500 text-sm">Item removed from wishlist.</p>
                    </div>
                    <button onClick={() => setShowRemoveAlert(false)} className="text-gray-400 hover:text-gray-600">✕</button>
                </div>,
                document.body
            )}
            ...
            <div className="relative aspect-square overflow-hidden rounded-xl bg-white p-3 cursor-pointer" onClick={handleProductClick}>
                <div className="w-full h-full transition-transform duration-700 ease-in-out group-hover/card:scale-110">
                    <img src={finalImageSrc} alt={product?.name} className="w-full h-full object-contain rounded-[20px]" />
                    {finalHoverImageSrc && (
                        <img src={finalHoverImageSrc} alt={product?.name} className="absolute inset-0 w-full h-full object-contain opacity-0 group-hover/card:opacity-100 transition-opacity duration-700 rounded-[20px]" /> 
                    )}
                </div>
            ...
            <div className="mt-3 flex flex-col flex-1">
                ...
                <div className="flex items-center gap-1 mt-1">
                    {[...Array(5)].map((_, i) => (
                        <MdStarPurple500 key={i} size={14} className={i < Math.round(rating) ? "text-yellow-400" : "text-gray-200"} />
                    ))}
                ...
                <div className="flex justify-between items-center mt-auto pt-3">
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                            <span className="text-lg font-bold text-[var(--primary)]">₹{finalPrice}</span>
                            {hasDiscount && <span className="text-red-500 text-[10px] font-bold">{discountPercent}% OFF</span>}
                        </div>
                        {hasDiscount && <span className="line-through text-gray-400 text-xs">₹{originalPrice}</span>}
                    </div>
