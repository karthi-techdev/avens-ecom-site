...
import {
  ...
  Smartphone,
  Menu,
  X, Laptop, Speaker, Footprints, Sun, Home, Baby, Shirt, Clock
} from "lucide-react";
...
const Header = () => {
  ...
  const handleSearch = async (e?: React.KeyboardEvent<HTMLInputElement>, overrideQuery?: string) => {
    ...
    if (queryToSearch.trim() !== "") {
      if (token) {
        await addSearch(queryToSearch.trim());
      }
    ...
    return (
      <div className={`absolute top-full left-0 mt-2 bg-white border border-gray-200 shadow-xl flex rounded-sm z-[100] 
        ...
        {selectedMainCategory && hasSubContent && (
          <div className={`w-3/4 p-8 grid gap-8 grid-cols-3`}>
            <div>
              {col1.map((sub) => (
                <div key={sub._id} className="mb-4">
                  ...
                  <ul className="space-y-2 text-sm text-gray-600">
                    {categories.filter(cat => cat.subCategoryId?._id === sub._id).map((item: any) => (
    ...
    <header className="w-full relative ">
      ...
      <div className="bg-white px-10">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-5 flex items-center justify-between">
          ...
          <div ref={searchRef} className="hidden lg:flex items-center w-full max-w-[700px] border-b-3 border-gray-800 bg-transparent relative h-[45px] ml-10">
            ...
            <div className="relative h-full">
              ...
              {listIsOpen && (
                <>
                  ...
                  <ul className="absolute left-0 top-full mt-2 w-full min-w-[200px] bg-white border border-gray-100 rounded-md shadow-xl z-20 max-h-80 overflow-y-auto custom-scrollbar py-2">
                    <li
                    ...
                    {maincategories.map((cat, index) => (
                      <li
                        ...
                        className="px-5 py-2 text-[13px] text-[#253D4E] hover:bg-[#F7F8F9] hover:text-[#3BB77E] cursor-pointer transition-colors"
                      >
                        {cat.name}
                      </li>
                    ))}
            ...
            <div className="flex-1 relative flex items-center h-full">
                <span className="text-gray-400 cursor-pointer" onClick={() => handleSearch()}>
                ...
                {showHistory && history.length > 0 && (
                  <ul className="absolute left-0 top-full mt-1 w-full bg-white border border-gray-100 rounded-md shadow-xl z-50 py-2">
                    <li className="px-5 py-2 flex justify-between items-center border-b border-gray-50 mb-1">
                    ...
                    {history.map((item) => (
                      <li
                        ...
                        className="group px-5 py-2 text-[13px] text-[#253D4E] hover:bg-[#F7F8F9] hover:text-[#3BB77E] cursor-pointer transition-colors flex items-center justify-between"
                      >
                        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-6">
              <div className="relative cursor-pointer">
                <Heart size={24} />
                <span className="absolute -top-2 -right-2 bg-[var(--primary)] text-white text-xs px-1.5 py-0.5 rounded-full">
                  4
                </span>
              </div>
              
              {/* USER ICON WITH REDIRECT LOGIC */}
            ...
            <Link href={isLoggedIn ? "/account" : "/login"}>
                <div className="relative cursor-pointer hover:text-[var(--primary)] transition-colors">
                  <FaRegUser size={24} />
                  <span className="absolute -top-2 -right-2 text-white text-xs px-1.5 py-0.5 rounded-full">
                  </span>
                </div>
              </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-4 flex justify-between items-center">
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 top-full mt-6 w-260 max-w-7xl bg-white shadow-xl rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 p-8">
                      <div className="grid grid-cols-4 gap-10">
                        ...
                        <div className="bg-gray-100 rounded-lg p-6 flex flex-col justify-between">
                          <div>
                            <p className="text-sm text-gray-500 uppercase">Hot Deals</p>
                            <h3 className="text-2xl font-bold mt-2">Don't miss <br /> Trending</h3>
                            <p className="text-[var(--primary)] font-semibold mt-2">Save to 50%</p>
                          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm">
              ...
              <strong className="text-gray-800">1900 – 888</strong>
            </div>
          </div>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-300 bg-white">
          <img src="/evara.svg" alt="Evaria Logo" className="h-8 w-auto object-contain" />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="p-4 border-b border-gray-300">
            <div className="flex border rounded-md overflow-hidden relative">
                  ...
                  <ul className="absolute left-0 top-full mt-1 w-full bg-white border border-gray-100 rounded-md shadow-xl z-50 py-2">
                    ...
                    {history.map((item) => (
                      <li
                        key={item._id}
                        onClick={() => handleHistoryClick(item.query)}
                        className="px-5 py-2 text-[13px] text-[#253D4E] hover:bg-[#F7F8F9] hover:text-[#3BB77E] cursor-pointer transition-colors flex items-center justify-between"
                      >
                        <div className="flex items-center gap-3">
                          <Clock size={14} className="text-gray-300" />
                          {item.query}
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-gray-200 rounded-full transition-all text-gray-400 hover:text-red-500"
                        >
                          <X size={14} />
                        </button>
                      </li>
              ...
              <button className="px-3 text-[var(--primary)]" onClick={() => handleSearch()}>
                <Search size={18} />
              </button>
            </div>
          </div>
